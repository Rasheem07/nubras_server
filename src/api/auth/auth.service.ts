...
export class AuthService {
  ...
  async salesmanRegister(username: string, contact: string, areaCode: string): Promise<{ type?: string; message?: string, statusCode?: number, accessToken?: string, refreshToken?: string }> {
    ...
    const newUser = await prisma.salesPerson.create({
      data: {
        name: username,
        areaCode: areaCode,
        contact: contact
  ...
  async verifySalesmanOTP(otpData: { otp: string, contact: string }) {
    const otp = await prisma.oTP.findFirst({
      where: {
        OTP: otpData.otp,
        contact: otpData.contact
    ...
    const existingUser = await prisma.salesPerson.findFirst({
      where: {
        contact: otpData.contact
      },
    });
    ...
    const token = jwt.sign({ type: 'salesman', id: existingUser?.id, isActive: existingUser?.isActive }, process.env.JWT_SECRET, { expiresIn: '15m' });
    const refreshToken = jwt.sign({ type: 'salesman', id: existingUser?.id, isActive: existingUser?.isActive }, process.env.JWT_SECRET, { expiresIn: '7d' });
